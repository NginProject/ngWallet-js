...
test('batch request with handlers', () => {
  ...
  const promise = rpc.batch([
    rpc.newBatchRequest('ngin_getBalance', ['0x01', 'latest'], (resp) => { balance1 = resp.result; }),
    rpc.newBatchRequest('ngin_getBalance', ['0x02', 'latest'], (resp) => { balance2 = resp.result; }),
    rpc.newBatchRequest('ngin_getBalance', ['0x03', 'latest'], (resp) => { balance3 = resp.result; }),
  ]);
...
test('batch request without handlers', () => {
  ...
  const promise = rpc.batch([
    rpc.newBatchRequest('ngin_getBalance', ['0x01', 'latest']),
    rpc.newBatchRequest('ngin_getBalance', ['0x02', 'latest']),
    rpc.newBatchRequest('ngin_getBalance', ['0x03', 'latest']),
  ]);
